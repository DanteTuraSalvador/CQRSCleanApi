    ...
    IErrorResponseService errorResponseService) : ControllerBase
{
    ...
    {
        var command = new CreateEstablishmentPhoneCommand(creationRequest);
        Result<EstablishmentPhoneResponse> result = await _dispatcher
            .SendCommandAsync<CreateEstablishmentPhoneCommand, Result<EstablishmentPhoneResponse>>(command);
        ...
        [FromBody] EstablishmentPhoneForUpdateRequest updateRequest)
    {
        ...
        var command = new UpdateEstablishmentPhoneCommand(phoneIdResult.Value!, updateRequest);
        Result<EstablishmentPhoneResponse> result = await _dispatcher
            .SendCommandAsync<UpdateEstablishmentPhoneCommand, Result<EstablishmentPhoneResponse>>(command);
        ...
        [FromBody] JsonPatchDocument<EstablishmentPhonePatchRequest> patchDocument)
    {
        ...
        var command = new PatchEstablishmentPhoneCommand(phoneIdResult.Value!, patchRequest);
        Result<EstablishmentPhoneResponse> result = await _dispatcher
            .SendCommandAsync<PatchEstablishmentPhoneCommand, Result<EstablishmentPhoneResponse>>(command);
    ...
    public async Task<IActionResult> DeleteEstablishmentPhone(string establishmentPhoneId)
    {
        ...
        var command = new DeleteEstablishmentPhoneCommand(phoneIdResult.Value!);
        Result result = await _dispatcher.SendCommandAsync<DeleteEstablishmentPhoneCommand, Result>(command);
        ...
        [FromQuery] bool? isPrimary = null)
    {
        if (!string.IsNullOrEmpty(establishmentPhoneId))
        {
            ...
            var getByIdQuery = new GetEstablishmentPhoneByIdQuery(phoneIdResult.Value!);
            Result<EstablishmentPhoneResponse> singlePhoneResult = await _dispatcher
                .SendQueryAsync<GetEstablishmentPhoneByIdQuery, Result<EstablishmentPhoneResponse>>(getByIdQuery);
        ...
        {
            var specWithPaging = new EstablishmentPhoneSpecification(
            ...
            var specWithoutPaging = new EstablishmentPhoneSpecification(
                establishmentId: establishmentId,
                phoneNumber: phoneNumber,
                isPrimary: isPrimary
            );

            var countQuery = new CountEstablishmentPhonesQuery(specWithoutPaging);
            Result<int> countResult = await _dispatcher
                .SendQueryAsync<CountEstablishmentPhonesQuery, Result<int>>(countQuery);

            if (!countResult.IsSuccess)
            { return HandleErrorResponse(countResult.ErrorType, countResult.Errors); }
            int totalCount = countResult.Value;

            var listQuery = new ListEstablishmentPhonesQuery(specWithPaging);
            Result<IEnumerable<EstablishmentPhoneResponse>> phonesResult = await _dispatcher
                .SendQueryAsync<ListEstablishmentPhonesQuery, Result<IEnumerable<EstablishmentPhoneResponse>>>(listQuery);
